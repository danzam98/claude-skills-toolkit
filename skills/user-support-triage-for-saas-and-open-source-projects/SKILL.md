...
This skill composes other skills (codebase-archaeology, comprehensive-codebase-report, github, admin-page-for-nextjs-sites, supabase, stripe-checkout, ga4, saas-customer-analytics, idea-wizard, user-support-ticketing-system-for-saas, e2e-testing-for-webapps, security-audit-for-saas, multi-model-triangulation). Before fan-out:
...
Borrowed from `/operationalizing-expertise`, `/sc`, and `/sw`: this skill is useful only if future agents can verify where every rule came from and can run the workflow without rediscovering context.
...
- `/codebase-archaeology` + `/comprehensive-codebase-report` — used inside Onboarding Phase 2.
- `/github` (`gh`) — used inside the GitHub fork.
...
| Generic GitHub issue/PR triage on OSS without customer context | `/managing-gh-issues-and-prs-with-ru` — leaner, no customer-comms layer |
...
| Cross-project pattern extraction across many projects | `/cross-project-pattern-extraction` — meta-level, runs above triage |
...
```bash
~/.claude/skills/sw/scripts/validate-skill.py \
  ~/.claude/skills/user-support-triage-for-saas-and-open-source-projects/
```
...
- "OSS-only GitHub triage with no customer-comms layer" → `/managing-gh-issues-and-prs-with-ru`
- "Mine prior triage sessions for what I asked" → `/cass`
- "Pattern extraction across many projects" → `/cross-project-pattern-extraction`
