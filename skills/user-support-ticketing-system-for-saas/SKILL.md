...
| Customer support runs on GitHub Issues only (OSS project) | `/managing-gh-issues-and-prs-with-ru` + triage skill's `GITHUB-FORK.md` |
...
| Final integration tests on real DB (no mocks) | `/testing-perfect-e2e-integration-tests-with-logging-and-no-mocks` |
...
- "Just write integration tests for an existing system" → `/testing-perfect-e2e-integration-tests-with-logging-and-no-mocks`
...
~/.claude/skills/sw/scripts/validate-skill.py \
  ~/.claude/skills/user-support-ticketing-system-for-saas/
```
