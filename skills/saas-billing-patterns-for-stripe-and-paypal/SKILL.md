...
If any helper skill referenced here is missing (`/sc`, `/sw`, `/operationalizing-expertise`, `/codebase-archaeology`, `/comprehensive-codebase-report`, `/security-audit-for-saas`, `/multi-pass-bug-hunting`, `/multi-model-triangulation`, `/saas-customer-analytics`, `/ubs`, `/ru-multi-repo-workflow`, `/agent-mail`, `/br`, `/bv`, `/vercel`, `/supabase`, `/stripe-checkout`, `/e2e-testing-for-webapps`, `/testing-real-service-e2e-no-mocks`, `/testing-perfect-e2e-integration-tests-with-logging-and-no-mocks`, `/idea-wizard`, `/cass`, `/cc-hooks`, `/github`, `/code-review-gemini-swarm-with-ntm`, `/multi-agent-swarm-workflow`): if the user has `jsm` installed and authenticated, offer to `jsm install <name>` for each missing one. Don't block a phase if a polish skill is missing — note it and proceed with the inline fallback in `references/methodology/SKILL-FALLBACKS.md`.
...
Full anti-pattern catalog with the bead trail for each: **[110-OPERATIONS.md § Patterns rejected](references/patterns/110-OPERATIONS.md#patterns-tried-and-rejected)**.
...
- **Corpus** — `/data/projects/je_private_skills_repo/COMPREHENSIVE_GUIDE_TO_SAAS_BILLING_PATTERNS_WITH_STRIPE_AND_PAYPAL.md` (78 sections, ~700 commits, 339 closed beads).
...
| Need | File |
|------|------|
| Master guide (78 sections) | [`COMPREHENSIVE_GUIDE_TO_SAAS_BILLING_PATTERNS_WITH_STRIPE_AND_PAYPAL.md`](../../../COMPREHENSIVE_GUIDE_TO_SAAS_BILLING_PATTERNS_WITH_STRIPE_AND_PAYPAL.md) |
