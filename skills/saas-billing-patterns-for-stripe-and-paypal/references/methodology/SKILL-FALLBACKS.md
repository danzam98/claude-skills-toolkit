...
The skill orchestrates multiple helper skills (`/sc`, `/sw`, `/operationalizing-expertise`, `/codebase-archaeology`, etc.). When one is missing, *fall back inline* — never block a phase on a missing skill.
...
```json
{
  "installed": ["sc", "sw", "operationalizing-expertise", "codebase-archaeology", "ubs", "agent-mail", "br"],
...
Inline fallback: not needed at run time; this skill (saas-billing-patterns) is itself the output. If a user wants to fork this skill for a different domain (e.g., billing-for-marketplace-with-Connect), point them to the repo-root best-practices document.

### `/sw` (missing)

Same: not needed at run time.
...
### `/comprehensive-codebase-report` (missing)
