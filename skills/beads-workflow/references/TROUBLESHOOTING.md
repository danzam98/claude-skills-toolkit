...
Some teams use git worktrees for sync operations. If your `sync-branch` points to your current branch, git cannot create a worktree for it.

NOTE: `br` itself never runs git commands. This error typically comes from a separate git/worktree-based sync workflow that you (or automation) are running around `.beads/issues.jsonl`.
...
|:--------|:-------------|
| `br sync --flush-only` | Export DB → JSONL (no git operations) |
| `br sync --import-only` | Import JSONL → DB (no git operations) |
| `br sync --status` | Sync status (read-only) |

### Check Sync Branch Configuration

```bash
br config get sync-branch          # Should NOT be your current branch (if using a sync-branch workflow)
...
| `bd sync` | `br sync --flush-only` | **Behavior change**: no auto-commit |
...
**Important:** `br sync --flush-only` only exports JSONL. You must `git add .beads/` and `git commit` manually.
...
br config set issue-prefix=myproject        # Set issue ID prefix
```
