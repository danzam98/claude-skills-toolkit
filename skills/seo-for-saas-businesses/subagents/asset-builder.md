...
5. **Design.** For benchmarks / datasets / calculators: chart specs (data + axis labels + dated source), social-card composition, OG image plan (delegated to `/og-share-images` or `/creating-share-images` if installed). For interactive tools: input UX and result-share UX (so embedded shares earn link-back).
