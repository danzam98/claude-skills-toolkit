...
| `core-update-response` | A confirmed broad core update overlaps the drop window | [CORE-UPDATE-PLAYBOOK](references/TRAFFIC-DROP-PLAYBOOK.md#core-update) — segmentation before rewrite |
...
| OG / Twitter image | `app/<route>/opengraph-image.tsx` and `twitter-image.tsx` via `next/og` `ImageResponse` (use `/og-share-images` or `/creating-share-images`) |
...
- `/github` skill for PR flow, milestones, and post-deploy ticket triage.
...
| Need | Skill |
|---|---|
| Generate dynamic OG/Twitter images | `/og-share-images`, `/creating-share-images` |
...
```bash
~/.claude/skills/sw/scripts/validate-skill.py /data/projects/je_private_skills_repo/.claude/skills/seo-for-saas-businesses/
```
