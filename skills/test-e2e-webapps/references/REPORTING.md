...
- [Cost Tracking Report](#cost-tracking-report)
...
| **Cost Report** | `.json` | AI API spend tracking | When using AI analysis |
...
export class ReportGenerator {
  ...
  addResult(result: TestResult): void {
    ...
    if (result.aiAnalysis) {
      this.aiCostCents += result.aiAnalysis.cost;
    }
  }
  ...
  private aggregateConsoleErrors(): Record<string, number> {
    // Aggregate console errors across all tests
...
<body>
  ...
  <div class="summary">
    ...
    <div class="stat cost">
      <h3>$${(report.costs.aiApiCents / 100).toFixed(2)}</h3>
      <p>AI API Cost</p>
    </div>
  </div>
...
### Dedicated Cost Reporter

```typescript
// e2e/utils/cost-reporter.ts
export interface CostReport {
  ...
  breakdown: {
    byModel: Record<string, { calls: number; costCents: number }>;
...
export class CostReporter {
  ...
  record(model: string, testName: string, costCents: number): void {
    this.costs.push({
      timestamp: new Date().toISOString(),
      model,
      testName,
  ...
  generateReport(): CostReport {
    const byModel: Record<string, { calls: number; costCents: number }> = {};
    ...
    for (const cost of this.costs) {
      ...
      if (!byModel[cost.model]) {
        byModel[cost.model] = { calls: 0, costCents: 0 };
      }
      byModel[cost.model].calls++;
      byModel[cost.model].costCents += cost.costCents;

      // By test
      byTest[cost.testName] = (byTest[cost.testName] || 0) + cost.costCents;

      // By day
      const day = cost.timestamp.split('T')[0];
      byDay[day] = (byDay[day] || 0) + cost.costCents;
    }

    const totalCents = this.costs.reduce((sum, c) => sum + c.costCents, 0);
    const days = Object.keys(byDay).length || 1;
    const runsPerDay = this.costs.length / days;

    return {
      period: {
        start: this.costs[0]?.timestamp || new Date().toISOString(),
        end: this.costs[this.costs.length - 1]?.timestamp || new Date().toISOString(),
      },
      totals: {
        aiApiCents: totalCents,
        screenshotsStored: this.costs.length,
        estimatedStorageMB: this.costs.length * 0.5,  // ~500KB per screenshot
      },
      breakdown: { byModel, byTest, byDay },
      projections: {
        monthlyEstimateCents: (totalCents / days) * 30,
  ...
  printSummary(): void {
    ...
    console.log(`Total: $${(report.totals.aiApiCents / 100).toFixed(2)}`);
    console.log(`Calls: ${this.costs.length}`);
    console.log(`Monthly projection: $${(report.projections.monthlyEstimateCents / 100).toFixed(2)}`);
    console.log('================================\n');
  }
}
```
...
```typescript
// In test setup or global teardown
...
export async function globalTeardown() {
  ...
  const budgetPercent = (summary.estimatedCostCents / summary.budgetCents) * 100;

  if (budgetPercent >= 90) {
    console.warn(`⚠️ AI BUDGET WARNING: ${budgetPercent.toFixed(0)}% used`);
    console.warn(`  Spent: $${(summary.estimatedCostCents / 100).toFixed(2)}`);
    console.warn(`  Budget: $${(summary.budgetCents / 100).toFixed(2)}`);
  }

  if (budgetPercent >= 100) {
    console.error('❌ AI BUDGET EXCEEDED - Some analyses were skipped');
  }
}
```
...
- name: Comment PR with results
  ...
  with:
    script: |
      ...
      | AI Cost | $${(results.costs.aiApiCents / 100).toFixed(2)} |
